...
router.get('/', async (req, res) => {
    try {
        const result = await db.query('SELECT * FROM event_categories ORDER BY name ASC');
...
router.post('/', authMiddleware, adminRequired, async (req, res) => {
    ...
    try {
        const result = await db.query(
            'INSERT INTO event_categories (name, slug) VALUES ($1, $2) RETURNING *',
            [name, slug]
        );
...
router.delete('/:id', authMiddleware, adminRequired, async (req, res) => {
    try {
        const result = await db.query('DELETE FROM event_categories WHERE id = $1 RETURNING *', [req.params.id]);
